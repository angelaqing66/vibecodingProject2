...
export default function PartnerProfilePage({
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setBookingError(
          result.error || 'Failed to book session. Please try again.'
        );
        setIsBooking(false);
        return;
      }
      ...
      setPartner((prev) =>
        prev
          ? {
              ...prev,
              availability: prev.availability.filter((a) => a !== selectedSlot),
            }
          : prev
      );
      setIsBooked(true);
            ...
            <div className="flex items-center gap-6 mb-8">
              ...
              <div>
                ...
                {partner.experienceLevel && (
                  <span
                    className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${getLevelColor(partner.experienceLevel)}`}
                  >
                    {partner.experienceLevel}
                  </span>
                )}
              ...
              {partner.availability && partner.availability.length > 0 ? (
                <div className="space-y-2">
                  {partner.availability.map((iso, idx) => (
                    <div
                      key={idx}
                      className="bg-purple-50 border border-purple-100 rounded-lg px-4 py-2.5 text-sm text-purple-800 font-medium"
                    >
                      📅 {formatSlot(iso)}
                    </div>
                  ))}
              ...
              <>
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex flex-col gap-2">
                      {(partner.interviewTypes || []).map((type) => (
                        <button
                          ...
                          className={`px-4 py-2.5 rounded-lg text-sm font-medium transition-all text-left border-2 ${
                            selectedType === type
                      ...
                      partner.availability.length > 0 ? (
                        partner.availability.map((iso) => (
                          <button
                            ...
                            className={`px-4 py-3 rounded-lg text-sm font-medium transition-all text-left border-2 ${
                              selectedSlot === iso
                                ? 'bg-purple-50 border-[#7C3AED] text-[#7C3AED]'
                  ...
                  <button
                    ...
                    className={`w-full px-6 py-3.5 rounded-xl font-bold text-white transition-all duration-200 flex items-center justify-center gap-2 ${
                      !selectedType || !selectedSlot || isBooking
