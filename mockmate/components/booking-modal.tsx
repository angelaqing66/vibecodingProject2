...
export function BookingModal({ partner, onClose }: BookingModalProps) {
    ...
    const slotsByDate = partner.availability.reduce((acc, slotIso) => {
    ...
    const sortedDates = Object.keys(slotsByDate).sort((a, b) => {
        return new Date(slotsByDate[a][0]).getTime() - new Date(slotsByDate[b][0]).getTime();
    });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-[#F3F4FE]/50 relative shrink-0">
                    <div>
                        <h2 className="text-xl font-extrabold text-[#0B1527]">Book a Session</h2>
                    ...
                    {status === 'success' ? (
                        <div className="flex flex-col items-center justify-center py-10 text-center space-y-4">
                            ...
                            <p className="text-gray-500 max-w-sm">
                                We&apos;ve notified {partner.name}. You can track the status in your Dashboard.
                            </p>
                            <button
                                onClick={onClose}
                                ...
                                {sortedDates.length === 0 ? (
                                    ...
                                    <div className="space-y-6">
                                        {sortedDates.map(dateLabel => (
                                            <div key={dateLabel}>
                                                <h4 className="text-sm font-semibold text-gray-500 mb-3 border-b border-gray-100 pb-1">{dateLabel}</h4>
                                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                                    {slotsByDate[dateLabel].sort((a, b) => new Date(a).getTime() - new Date(b).getTime()).map(slotIso => {
                                                        const timeStr = new Intl.DateTimeFormat('en-US', { hour: 'numeric', minute: '2-digit' }).format(new Date(slotIso));
