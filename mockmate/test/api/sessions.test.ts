...
describe('POST /api/sessions', () => {
    ...
    it('returns 401 when not authenticated', async () => {
        ...
        const res = await POST(makeRequest({ hostId: 'h1', scheduledTime: new Date().toISOString() }));
    ...
    it('returns 409 when slot already booked', async () => {
        ...
        const res = await POST(makeRequest({ hostId: 'host-1', scheduledTime: slotISO }));
    ...
    it('returns 500 on unexpected DB error', async () => {
        ...
        vi.mocked(prisma.user.findUnique).mockRejectedValueOnce(new Error('DB crash'));
...
describe('GET /api/sessions', () => {
    ...
    it('returns 200 with sessions list for authenticated user', async () => {
        ...
        vi.mocked(prisma.mockSession.findMany).mockResolvedValueOnce(mockSessions as any);
