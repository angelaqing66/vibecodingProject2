...
test.describe('Partner Search — UI & Filtering', () => {
  ...
  test('SEARCH-02 — Search page renders partner cards after login', async ({
    page,
  }) => {
    await loginAs(
      page,
    ...
    await expect(page.locator('h1')).toContainText(
      /Find a Mock Interview Partner/i,
      { timeout: 8000 }
    );
  });

  test('SEARCH-03 — Search input is present and accepts text', async ({
    page,
  }) => {
    await loginAs(
      page,
  ...
  test('SEARCH-04 — Experience level filter renders options', async ({
    page,
  }) => {
    await loginAs(
      page,
    ...
    const filterSelect = page
      .locator('select, [data-testid="level-filter"]')
      .first();
  ...
  test('SEARCH-05 — Interview type filter renders', async ({ page }) => {
    ...
    const typeFilter = page
      .locator('select, [data-testid="type-filter"]')
      .first();
    await expect(typeFilter).toBeVisible({ timeout: 8000 });
  });

  test('SEARCH-06 — View Profile button navigates to partner profile page', async ({
    page,
  }) => {
    await loginAs(
      page,
  ...
  test('SEARCH-07 — Partner profile page has booking widget', async ({
    page,
  }) => {
    await loginAs(
      page,
  ...
  test('SEARCH-08 — Back to Search button works from partner profile', async ({
    page,
  }) => {
    await loginAs(
      page,
