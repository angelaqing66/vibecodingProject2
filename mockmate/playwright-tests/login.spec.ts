...
test.describe('Authentication — Login & Signup', () => {
    ...
    test('AUTH-03 — Login shows error on invalid credentials', async ({ page }) => {
    ...
    test('AUTH-06 — Unauthenticated user is redirected from /dashboard to /login', async ({ page }) => {
        await page.goto('/dashboard');
        await expect(page).toHaveURL(/login/);
    });

    test('AUTH-07 — Unauthenticated user is redirected from /search to /login', async ({ page }) => {
        await page.goto('/search');
        await expect(page).toHaveURL(/login/);
    });

    test('AUTH-08 — Unauthenticated user is redirected from /profile to /login', async ({ page }) => {
        await page.goto('/profile');
        await expect(page).toHaveURL(/login/);
    });

    test('AUTH-09 — Unauthenticated user is redirected from /admin to /login', async ({ page }) => {
