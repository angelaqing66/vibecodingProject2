...
test.describe('Dashboard — Session Management', () => {
    ...
    test('DASH-02 — Dashboard header and tabs render correctly', async ({ page }) => {
        await loginAs(page, process.env.TEST_USER_EMAIL!, process.env.TEST_USER_PASSWORD!);
        await page.goto('/dashboard');
        await expect(page.locator('h1', { hasText: 'Your Dashboard' })).toBeVisible({ timeout: 8000 });
    ...
    test('DASH-03 — Find Partners button navigates to search', async ({ page }) => {
        await loginAs(page, process.env.TEST_USER_EMAIL!, process.env.TEST_USER_PASSWORD!);
        await page.goto('/dashboard');
    ...
    test('DASH-04 — Pending tab shows correct empty state', async ({ page }) => {
        ...
        await expect(page.locator('text=No pending')).toBeVisible({ timeout: 5000 })
            .catch(() => { /* has pending sessions — also fine */ });
    });
...
test.describe('Profile Setup Flow', () => {
    ...
    test('PROF-03 — Profile page shows experience level options', async ({ page }) => {
        await loginAs(page, process.env.TEST_USER_EMAIL!, process.env.TEST_USER_PASSWORD!);
        // Visit profile or profile-setup
        const dest = await page.goto('/profile');
        if (dest?.url().includes('login')) { test.skip(); return; }
        // Expect experience level options to be present
        const experienceOptions = page.locator('text=New Grad, text=Intern, text=Experienced').first();
        await expect(experienceOptions.or(page.locator('text=Experience Level'))).toBeVisible({ timeout: 8000 });
    });
});

test.describe('Navigation & Navbar', () => {

    test('NAV-01 — Navbar renders top-level links after login', async ({ page }) => {
        await loginAs(page, process.env.TEST_USER_EMAIL!, process.env.TEST_USER_PASSWORD!);
        await page.goto('/dashboard');
    ...
    test('NAV-02 — 404 page renders for unknown route', async ({ page }) => {
        ...
        const is404 = await page.locator('text=Page Not Found').isVisible({ timeout: 5000 }).catch(() => false);
