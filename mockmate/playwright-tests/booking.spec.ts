...
test.describe('Session Booking Flow', () => {
    ...
    test('BOOK-01 — Confirm Booking button is disabled without selections', async ({ page }) => {
        ...
        await expect(page.locator('[data-testid="confirm-booking"]')).toBeDisabled({ timeout: 8000 });
    });

    test('BOOK-02 — Booking button stays disabled with only interview type selected', async ({ page }) => {
        await loginAs(page, process.env.TEST_USER_EMAIL!, process.env.TEST_USER_PASSWORD!);
        await page.goto('/search');
        const viewProfileBtn = page.locator('text=View Profile').first();
        const isVisible = await viewProfileBtn.isVisible({ timeout: 8000 }).catch(() => false);
        ...
        const typeVisible = await firstType.isVisible({ timeout: 8000 }).catch(() => false);
    ...
    test('BOOK-03 — Booking button becomes enabled after both selections', async ({ page }) => {
        ...
        if (!bothVisible.every(Boolean)) { test.skip(); return; }
    ...
    test('BOOK-04 — Optional notes field accepts text', async ({ page }) => {
        ...
        const notesVisible = await notes.isVisible({ timeout: 8000 }).catch(() => false);
        if (!notesVisible) { test.skip(); return; }

        await notes.fill('Looking forward to practicing linked list problems!');
        await expect(notes).toHaveValue('Looking forward to practicing linked list problems!');
    });
    ...
    test('BOOK-05 — Full booking flow shows success state', async ({ page }) => {
        ...
        const success = await page.locator('[data-testid="success-state"]').isVisible({ timeout: 10000 }).catch(() => false);
        const errorMsg = await page.locator('[data-testid="booking-error"]').isVisible({ timeout: 10000 }).catch(() => false);

        // Either success or a meaningful error is acceptable
        expect(success || errorMsg).toBeTruthy();
    });

    test('BOOK-06 — Success state has View in Dashboard button', async ({ page }) => {
        await loginAs(page, process.env.TEST_USER_EMAIL!, process.env.TEST_USER_PASSWORD!);
        await page.goto('/search');
        const viewProfileBtn = page.locator('text=View Profile').first();
        const isVisible = await viewProfileBtn.isVisible({ timeout: 8000 }).catch(() => false);
        ...
        const successVisible = await page.locator('[data-testid="success-state"]').isVisible({ timeout: 10000 }).catch(() => false);
        if (!successVisible) { test.skip(); return; }
