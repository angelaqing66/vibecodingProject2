...
function DashboardContent() {
  ...
  const initialTab =
    (searchParams.get('tab') as 'upcoming' | 'pending' | 'past') || 'upcoming';
  ...
  const [activeTab, setActiveTab] = useState<'upcoming' | 'pending' | 'past'>(
    () => {
      ...
      return tab && ['upcoming', 'pending', 'past'].includes(tab)
        ? tab
        : initialTab;
    }
  );
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center">
          <div>
            ...
            <p className="text-gray-500">
              Manage your mock interviews and requests
            </p>
          </div>
          <button
        ...
        <div className="flex space-x-2 bg-white p-2 rounded-2xl shadow-sm border border-gray-100 w-full overflow-x-auto">
          <button
            ...
            className={`flex-1 py-3 px-4 rounded-xl font-semibold text-sm transition-colors whitespace-nowrap ${
              activeTab === 'upcoming'
            ...
            <div className="space-y-4">
              {upcoming.length === 0 ? (
                ...
                upcoming.map((session) => (
                  <div
                    key={session.id}
                    className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between gap-4"
                  >
                    <div>
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        {pendingReceived.map((session) => (
                          <div
                            key={session.id}
                            className="bg-white rounded-2xl p-6 border-l-4 border-[#8A2BE2] shadow-sm flex flex-col md:flex-row justify-between gap-4"
                          >
                            <div>
                              <p className="text-[#8A2BE2] font-semibold text-sm mb-1">
                                {formatDateTime(session.scheduledTime)}
                              </p>
                              ...
                              <p className="text-gray-500 text-sm mb-3">
                                {session.guest?.experienceLevel} •{' '}
                                {session.guest?.interviewTypes?.join(', ')}
                              </p>
                              {session.message && (
                    ...
                    <div className="pt-4">
                      ...
                      <div className="space-y-4">
                        {pendingSent.map((session) => (
                          <div
                            key={session.id}
                            className="bg-white/60 rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between gap-4 opacity-80"
                          >
                            <div>
                              ...
                              <p className="text-gray-500 text-sm mb-3">
                                {session.host?.experienceLevel} •{' '}
                                {session.host?.interviewTypes?.join(', ')}
                              </p>
                              {session.message && (
            ...
            <div className="space-y-4">
              {past.length === 0 ? (
                ...
                past.map((session) => (
                  <div
                    key={session.id}
                    className="bg-gray-50 rounded-2xl p-6 border border-gray-100 flex flex-col justify-between gap-2 opacity-70"
                  >
