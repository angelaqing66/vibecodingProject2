...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function fetchProfile() {
      ...
      if (res.success && res.user) {
        setValue(
          'experienceLevel',
          res.user.experienceLevel as 'Intern' | 'New Grad' | 'Experienced'
        );
    ...
    try {
      ...
      if (!isNaN(dateObj.getTime())) {
        ...
        if (!current.includes(isoString)) {
          setValue('availability', [...current, isoString], {
            shouldValidate: true,
          });
        }
      }
  ...
  const removeAvailability = (isoString: string) => {
    ...
    setValue(
      'availability',
      current.filter((time) => time !== isoString),
      { shouldValidate: true }
    );
  };
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
        <div className="bg-[#8A2BE2] py-8 px-6 sm:px-10 text-center">
          ...
          <p className="text-purple-100/80 text-lg">
            Manage your settings and availability
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
            ...
            <section className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-2">
                  What kind of mock interviews do you want to do?
                </p>
              </div>
            ...
            <section className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Add the exact dates and times you are free to do a mock
                  interview.
                </p>
              </div>
