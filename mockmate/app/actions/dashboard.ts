...
export async function getDashboardData() {
  ...
  try {
    ...
    const pendingSent = allSessions.filter(
      (s) =>
        s.guestId === userId && s.status === 'PENDING' && s.scheduledTime > now
    );

    const pendingReceived = allSessions.filter(
      (s) =>
        s.hostId === userId && s.status === 'PENDING' && s.scheduledTime > now
    );

    const upcoming = allSessions.filter(
      (s) => s.status === 'SCHEDULED' && s.scheduledTime > now
    );
    ...
    const past = allSessions.filter(
      (s) =>
        ...
        (s.status === 'SCHEDULED' ||
          s.status === 'COMPLETED' ||
          (s.status === 'PENDING' && s.guestId === userId)) // Show missed pending sent
    );
...
export async function declineSession(sessionId: string) {
  ...
  try {
    ...
    if (
      mockSession.hostId !== session.user.id &&
      mockSession.guestId !== session.user.id
    ) {
